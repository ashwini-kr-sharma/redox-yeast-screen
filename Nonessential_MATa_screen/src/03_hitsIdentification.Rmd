...
### Topmost 5% hits as Venn diagram

```{r}
par(mar = c(1,1,1,1), xpd = T)
venn::venn(topHits[1:6], box = F, zcolor = brewer.pal(6, "Set1"), plotsize = 30)

pdf(paste0(path, 'analysis/tophits/venn_TOP_5percent_all_categories.pdf'), width = 7, height = 5)
venn::venn(topHits[1:6], box = F, zcolor = brewer.pal(6, "Set1"), plotsize = 30)
invisible(dev.off())
```

### Topmost 5% hits as UpSet plot

```{r, fig.width=10}
upset(fromList(topHits), nsets = 12, keep.order = T, sets = names(topHits)[c(6:1)], 
      nintersects = NA, number.angles = 30, order.by = "degree", mainbar.y.label = "Overlap counts - top 5% roGFP2 ratio",
      sets.x.label = "Number of mutants")
```

### Lowest 5% hits as Venn diagram

```{r}
par(mar = c(1,1,1,1), xpd=T)
venn::venn(topHits[7:12], box = F, zcolor = brewer.pal(6, "Set1"), plotsize = 30)

pdf(paste0(path, 'analysis/tophits/venn_LOW_5percent_all_categories.pdf'), width = 7, height = 5)
venn::venn(topHits[7:12], box = F, zcolor = brewer.pal(6, "Set1"), plotsize = 30)
invisible(dev.off())
...
```{r, fig.width=10}
upset(fromList(topHits), nsets = 12, keep.order = T, sets = names(topHits)[c(7:12)], 
      nintersects = NA, number.angles = 30, order.by = "degree", mainbar.y.label = "Overlap counts - lowest 5% roGFP2 ratio",
      sets.x.label = "Number of mutants")
...
tmpdat = mito[mito$Genes %in% topHits_allcarbon$common_hits$Mitochondria_high_roGFP2_commom,]
...
```{r}
tmpdat = mito[mito$Genes %in% topHits_allcarbon$common_hits$Mitochondria_low_roGFP2_commom,]
...
# Genes to plot
manisha_genes = c("SOD2", "GND1", "RPE1", "TSA1", "IDP3", "TRX1", "SOD1", "GRX1")
#-------------------

type_norm = rep("Others", nrow(normDat$redox_table))
type_norm[normDat$redox_table$Genes %in% rdx] = "Redox"
dat = cbind(normDat$redox_table, type = type_norm)

# Normalized data
ggplot(dat, aes(Median_roGFP2_ratio, fill = type)) + theme_bw(base_size = 9) +
labs(x = "Normalized roGFP2 ratio", y ="Density") +
geom_density(alpha=0.25) + xlim(-8,8) +
facet_wrap(Nutrient ~ Organelle, scales = "free", ncol = 2) + geom_vline(xintercept = c(-1,1)) +
geom_text_repel(data = dat[dat$Genes %in% manisha_genes,], aes(y=0, label = Genes), nudge_y = 0.15) +
theme(panel.grid = element_blank())

ggsave(filename = paste0(path,"analysis/tophits/known_poistive_hits_selected_manisha.pdf"), width = 8, height = 8)
