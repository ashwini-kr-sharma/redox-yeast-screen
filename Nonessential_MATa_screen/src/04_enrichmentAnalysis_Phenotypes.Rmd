---
title: "Enrichment analysis - Phenotypes"
...
cytoOX = lapply(cyto, function(x){
                                  ...
                                  x = computeEnrichment(gset = genesets$Phenotype, genes = selgenes)

                                  # Exactly same as in hypeR package
                                  # For sanity check of my own implemented function !!
                                  
                                  # x = hypeR(signature = selgenes[selgenes %in% unique(unlist(genesets$Phenotype))],
                                  #           gsets = genesets$Phenotype,
                                  #           test = "hypergeometric",
                                  #           bg = unique(unlist(genesets$Phenotype)),
                                  #           pval_cutoff = 0.05,
                                  #           fdr_cutoff = 0.25,
                                  #           verbose = T)$data
...
```{r fig.width = 8, fig.height = 20, fig.align="center"}
...
pheatmap(
  ...
  annotation_row = anno_rows,
  annotation_colors = anno_color,
  filename = paste0(path, "analysis/enrichment/top_25percent_hits_Phenotypes_enrichment_hypergeometricTest.pdf"),
  width = 7, height = 17)
```
...
Unlike other genesets (`GPBP, GOMF, KEGG, TFs`), we get many significant enrichment terms for `Phenotypes`, likely because they consist relatively of lesser number of genes per geneset. Thus, we don't show the fold enrichment and frcation overlap plots for `Phenotypes`.
