---
title: "Enrichment analysis - TF targets"
...
across curated set of TF target genes
...
cytoOX = lapply(cyto, function(x){
                                  ...
                                  x = computeEnrichment(gset = genesets$TFtargets, genes = selgenes)

                                  # Exactly same as in hypeR package
                                  # For sanity check of my own implemented function !!
                                  
                                  # x = hypeR(signature = selgenes[selgenes %in% unique(unlist(genesets$TFtargets))],
                                  #           gsets = genesets$TFtargets,
                                  #           test = "hypergeometric",
                                  #           bg = unique(unlist(genesets$TFtargets)),
                                  #           pval_cutoff = 0.05,
                                  #           fdr_cutoff = 0.25,
                                  #           verbose = T)$data
...
pheatmap(
  ...
  annotation_row = anno_rows,
  annotation_colors = anno_color,
  filename = paste0(path, "analysis/enrichment/top_5percent_hits_TFtargets_enrichment_hypergeometricTest.pdf"),
  width = 5, height = 3)
}
```
...
```{r fig.width = 5, fig.height = 14, fig.align="center"}
...
pheatmap(
  ...
  annotation_row = anno_rows,
  annotation_colors = anno_color,
  filename = paste0(path, "analysis/enrichment/top_5percent_hits_TFtargets_Foldenrichment.pdf"),
  width = 5, height = 15)
...
col.cluster =  c(brewer.pal(length(levels(anno_rows$Clusters)), "Paired"), "grey50")
names(col.cluster) = as.character(levels(anno_rows$Clusters))
...
pheatmap(
  ...
  annotation_row = anno_rows,
  annotation_colors = anno_color,
  filename = paste0(path, "analysis/enrichment/top_5percent_hits_TFtargets_FractionOverlap.pdf"),
  width = 5, height = 15)
```
